...
// appliedTailSize is an approximation of the size of the applied entries.
//
// called with p locked.
func (p *fsPersist) timeToSnapshot() bool {
	p.r.applyLock.Lock()
	defer p.r.applyLock.Unlock()
	return int64(p.r.lastIndexApplied - p.firstLogTailIndex) > p.snapshotThreshold
}
...
func (p *fsPersist) AppendToLog(ctx *context.T, prevTerm Term, prevIndex Index, entries []LogEntry) error {
	...
	// Take a snapshot if the log it too big.
	if p.timeToSnapshot() && !p.snapping {
		p.snapping = true
		go p.TakeSnapshot(ctx)
	}
...
// TakeSnapshot implements Persistence.TakeSnapshot.
func (p *fsPersist) TakeSnapshot(ctx *context.T) error {
	r := p.r
	defer func() { p.snapping = false }()
	// Create a file for the snapshot.  Lock to prevent any logs from being aplied while we do this.
	r.applyLock.Lock()
	t, i := r.lastTermApplied, r.lastIndexApplied
	...
	// Wait for the snopshot to finish.
	if err := <-c; err != nil {
		fp.Close()
		os.Remove(fn)
		vlog.Errorf("snapshot %s: %s", fn, err)
...
func (p *fsPersist) SnapshotFromLeader(ctx *context.T, term Term, index Index, call raftProtoInstallSnapshotServerCall) error {
	...
	defer fp.Close()
	if err := r.client.RestoreFromSnapshot(ctx, fp); err != nil {
		return err
	}
	r.lastTermApplied, r.lastIndexApplied = term, index
	p.Lock()
...
func (p *fsPersist) addToLogTail(le *LogEntry) bool {
...
func (p *fsPersist) readState(ctx *context.T, r *raft) error {
	...
	for i := len(snaps)-1; i >= 0; i-- {
		...
		err = p.client.RestoreFromSnapshot(ctx, fp)
