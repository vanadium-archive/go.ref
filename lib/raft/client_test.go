...
func (c *client) Apply(le *LogEntry) error {
	vlog.Infof("Applying %d/%d %s", le.Term, le.Index, le.Cmd)
	c.cmds = append(c.cmds, le.Cmd)
	return nil
}
...
func (c *client) RestoreFromSnapshot(ctx *context.T, rd io.Reader) error {
...
func buildRafts(t *testing.T, ctx *context.T, n int, config *RaftConfig) []*raft {
	if config == nil {
	...
	var td []string
	for i := 0; i < n; i++ {
		if n > 1 || len(config.LogName) == 0 {
			config.LogName = tempDir(t)
		}
		r, err := newRaft(ctx, config, new(client))
		if err != nil {
			t.Fatalf("NewRaft: %s", err)
		}
		td = append(td, config.LogName)
		rs = append(rs, r)
...
func restart(t *testing.T, ctx *context.T, rs []*raft, r *raft) {
	config := RaftConfig{HostPort: r.me.id[1:], LogName: r.logDir}
	rn, err := newRaft(ctx, &config, new(client))
	if err != nil {
		t.Fatalf("NewRaft: %s", err)
	}
	for i := range rs {
...
func TestClientSnapshot(t *testing.T) {
	...
	c := new(client)
	for _, cmd := range([]string{"the", "rain", "in", "spain", "falls", "mainly", "on", "the", "plain"}) {
		c.Apply(&LogEntry{Cmd:[]byte(cmd)})
	}
	...
	if err := nc.RestoreFromSnapshot(ctx, fp); err != nil {
