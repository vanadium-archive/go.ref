...
func TestCreation(t *testing.T) {
	...
	config := RaftConfig{HostPort: "127.0.0.1:0", LogName: td}
...
func TestSnapshot(t *testing.T) {
	...
	config := RaftConfig{HostPort: "127.0.0.1:0", LogName: td, SnapshotThreshold: 1}
	rs := buildRafts(t, ctx, 1, &config)
	defer cleanUp(rs)
	r := rs[0]
	c1 := r.client
	vlog.Infof("id is %s", r.Id())
...
func TestRemoteSnapshot(t *testing.T) {
	...
	rs := buildRafts(t, ctx, 3, &config)
	defer cleanUp(rs)

	// This should cause a few snapshots.
	for i := 0; i < 100; i++ {
		if apperr, err := rs[0].Append(ctx, []byte(fmt.Sprintf("string%d", i))); err != nil || apperr != nil {
	...
	t.Log("Logs agree")
	if !waitForAppliedAgreement(rs, time.Minute) {
