...
main() {
  ...
  shell::run_server "${shell_test_DEFAULT_SERVER_TIMEOUT}" "${MTLOG}" "${MTLOG}" \
    "${VRUN}" "${MOUNTTABLED_BIN}" --veyron.tcp.address=127.0.0.1:0 -vmodule=publisher=2 &> /dev/null \
  ...
  shell::run_server "${shell_test_DEFAULT_SERVER_TIMEOUT}" "${TUNLOG}" "${TUNLOG}" \
    "${VRUN}" "${TUNNELD_BIN}" --veyron.tcp.address=127.0.0.1:0 -vmodule=publisher=2 &> /dev/null \
